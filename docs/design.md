...
2. Pick a **`measure` row not meeting its target** (`meets_target: false`; every FAIL before any unaccepted WATCH, then a fixed tie-break order — see [improve.md](../skills/docgrad/reference/improve.md) §Working set), and generate a batch of focused fixes from that row's deductions (each round fixes only one signal, to avoid half-finished changes across everything that leave contradictions — convergence is not a rewrite). A judge star, when `--judge` ran, is never what gets picked.
